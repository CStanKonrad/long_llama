...
from FoT.cross_batch import cross_batch_attention
import functools
...
LLAMA_STANDARD_CONFIGS = {
    ...
    'debug_fot': { # A small model for debugging
        'vocab_size': 32000,
        'hidden_size': 128,
        'intermediate_size': 256,
        'num_hidden_layers': 2,
...
class LLaMAConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.cross_batch_stepping = cross_batch_stepping
        self.dataset_packing = dataset_packing
        self.flip_sharding_in_cross_batch = flip_sharding_in_cross_batch
        self.scan_cross_batch = scan_cross_batch
...
class FlaxLLaMAAttention(nn.Module):
    ...
    def setup(self):
        ...
        self.cross_batch_layer = self.memory_layer and self.config.cross_batch_range > 0
    ...
    def __call__(
        ...
        if init_cache and self.cross_batch_layer:
            raise ValueError("Cache not supported in FoT training code")
        
        xq, xk, xv = self.wq(hidden_states), self.wk(hidden_states), self.wv(hidden_states)

        if self.cross_batch_layer and self.config.flip_sharding_in_cross_batch:
            xq = with_sharding_constraint(xq, PS(("dp", "mp"), None, ("fsdp")))
            xk = with_sharding_constraint(xk, PS(("dp", "mp"), None, ("fsdp")))
            xv = with_sharding_constraint(xv, PS(("dp", "mp"), None, ("fsdp")))
        else:
        ...
        min_value = jnp.asarray(-1e6, dtype=self.dtype)
        if self.config.scan_attention and not (self.has_variable("cache", "cached_key") or init_cache) and not self.cross_batch_layer:
            ...
            attention_bias = lax.select(
                ...
                jnp.full(attention_mask.shape, min_value).astype(self.dtype),
            )
            attn_weights = None
            ...
            if self.cross_batch_layer:
                logger.info(f"Using cross batch in {self.name} with cross_batch_range={self.config.cross_batch_range} cross_batch_stepping={self.config.cross_batch_stepping} dataset_packing={self.config.dataset_packing}")
                
                attn_output = cross_batch_attention(
                    query=xq,
                    key=xk,
                    value=xv,
                    bias=attention_bias,
                    min_value=min_value,
                    cross_batch_range=self.config.cross_batch_range,
                    cross_batch_stepping=self.config.cross_batch_stepping,
                    dataset_packing=self.config.dataset_packing,
                    pos_encode_as_first=functools.partial(apply_rotary_emb_as_fst, freqs_cis=freqs_cis, dtype=self.dtype),
                    pos_encode=functools.partial(apply_rotary_emb, freqs_cis=freqs_cis, dtype=self.dtype),
                    dropout_rng=dropout_rng,
                    dropout_rate=self.config.attn_pdrop,
                    deterministic=deterministic,
                    dtype=jnp.promote_types(self.dtype, jnp.float32),
                    precision=self.precision,
                    custom_attention_fn=blockwise_attn if self.config.scan_cross_batch else None
                )
...
class FlaxLLaMABlock(nn.Module):
    ...
    def setup(self) -> None:
        if self.memory_layer:
            logger.info(f"Block {self.name} will have cross batch with parameters cross_batch_range={self.config.cross_batch_range} cross_batch_stepping={self.config.cross_batch_stepping} dataset_packing={self.config.dataset_packing}")
    ...
    def setup(self):
        ...
        self.blocks = [
            block(
                ...
                param_dtype=self.param_dtype,
                precision=self.precision,
                memory_layer=i in self.config.mem_layers
